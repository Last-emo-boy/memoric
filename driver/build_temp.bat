...
cl.exe /kernel /W4 /WX- /O2 /GS- /Gz ^
    /D _AMD64_ /D AMD64 /D _WIN64 /D NTDDI_VERSION=0x0A000007 ^
    /I "%WDK_INC%\km" ^
    /I "%WDK_INC%\shared" ^
    /I "%WDK_INC%\um" ^
    /I "." ^
...
if errorlevel 1 (
    ...
    signtool sign /v /a /s PrivateCertStore /n MemoricTestCert /fd sha256 memoric_new.sys
    if errorlevel 1 (
...
echo.
echo [+] Build complete: memoric.sys (37 IOCTLs)
echo [+] Size: 
